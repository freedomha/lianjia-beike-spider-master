    ...
    def get_area_ershou_info(city_name, area_name):
        ...
        for num in range(1, total_page + 1):
            ...
            for house_elem in house_elements:
                ...
                hid = house_elem.find('div', class_="unitPrice")
                subway = house_elem.find('div', class_="tag")

                # 继续清理数据
                price = price.text.strip().replace("\n", "").strip()
                position = position.text.replace("\n", "").strip()
                name = name.text.replace("\n", "").replace(",", "，")
                desc = desc.text.replace("\n", "").replace(" ","").replace("|",",").replace("平米","").strip()
                ...
                # 作为对象保存
                ershou = ErShou(hid, chinese_district, chinese_area, position, name, price, unitPrice ,followInfo, desc, subway, pic)
